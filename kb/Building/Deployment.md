...
| Local container | `docker compose up` (from `docker/`) | Working with sibling `../converge` checkout |
| Google Cloud Run (runtime) | `ops/scripts/deploy-cloud-run.sh` | Script-based; stages sibling `../converge` source |
...
By default, the Docker build reads runtime source from sibling `../../converge`.
...
Requires: `gcloud` CLI, `PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` env var, and a local `../converge` checkout (or `CONVERGE_ROOT`).
...
- Local SDK development can patch those crates back to sibling `../converge`
- Runtime helper scripts resolve runtime source from sibling `../converge` or `CONVERGE_ROOT`
